...
- 典型结构：`features/` + `components/` + `lib/` + `app/`
- 公共 vs 私有：什么该提取到通用层
- 单体 vs Monorepo（多包管理，大型项目）
- 目录约定的价值（约定优于配置）

### 2. 状态架构（State Architecture）
- 状态分层：UI 状态 / 服务端状态 / URL 状态 / 持久状态（→ 详见 01-5）
- 状态归属决策：哪些全局、哪些局部、哪些 URL
- API 层 + Hook 层 + 状态层的分离
- 避免状态重复存储（单一数据源）
...
### 6. 工程规范（Engineering Conventions）
